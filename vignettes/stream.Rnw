...
of \pkg{stream} and and focus on its use for data stream clustering research. 
\pkg{stream}
...
Very recently \pkg{RMOA}~\citep{stream:Wijffels:2014} was introduced. Th package interfaces data stream classification algorithms from the MOA framework (see existing tools discussed in Section~\ref{sec:background:moa}), however, the package focuses on static data sets that do not fit into main memory.
...
package \pkg{stream}~\citep{stream:stream:2014}.
...
Next, we create an instance of the data stream clustering algorithm 
D-Stream and update the model with the next 500 data points from the stream.
<<>>=
...
framework which are of interest to a users.
...
Matrix-like objects also includes large objects potentially stored on disk
like \code{ffdf} from
...
\item \code{DSD_ReadCSV} reads data line by line in CSV format
...
Connections can be used to read from real-time data streams (blocking or non-blocking).
...
implemented. For example, \code{DSD_Memory} and \code{DSD_ReadCSV}
...
Figures~\ref{figure:static_pc} show 
the projected data. 
...
Using \code{take} and \code{class} we define which columns should be used
...
need to be specified if they are not available or no evaluation is planned. Note that at this point no data has been read in.
...
two types of DSD classes can also be reset to the beginning of the stream
 or to an arbitrary position via \code{reset_stream()}. Here we set the 
...
combine any available micro and a macro-clustering algorithm.
...
\item
\code{update(dsc, dsd, n = 1, verbose = FALSE, ...)} which accepts a DSC object and a DSD object. It requests the next $n$
...
in \code{points} would be assigned. For assignment, 
...
\code{type = c("auto", "micro", "macro", "both")} 
...
Algorithm~2.1 by \cite{stream:Aggarwal:2006} is also implemented.

\item\code{DSC_DBSTREAM} implements the simple data stream clustering algorithm called 
...
Some data clustering algorithms create small clusters for noise or 
...
The current state
of the evaluation of data stream clustering is described 
...
\subsection{Evaluating the learned static structure}

Evaluation 
how well an algorithm is able to learn a static structure in the data 
...
used for evaluation.
The points are assigned to the 
...
\item Internal evaluation measures.
	\begin{itemize}
		\item	\code{"SSQ"} Within cluster sum of squares (actual noise points
    identified by the clustering algorithm are excluded) 
		\item	\code{"silhouette"} Average silhouette width (actual noise points
    identified by the clustering algorithm are excluded) (\pkg{cluster}) 
...
data points just form a special group in the partition. 
However, for 
some internal 
...
of squares. Therefore, for some internal measures, noise points are excluded.
...
\subsection{Evaluating clustering of dynamic streams}
...
have developed an evaluation scheme for data stream clustering which 
...
a number of data points.
Each data point in the horizon is used for clustering and then it is evaluated 
how well the 
...
By default for a \code{DSD_Micro} implementation like D-Stream, 
the data points are assigned to micro clusters and then this assignment is 
...
cluster. Note that the silhouette is only calculated for a small number of points and not the whole stream.
...
Cluster $j=1$ comprises the two overlapping real clusters and thus
has lower silhouette values than cluster $j=2$. Other visual evaluation methods
...
with 5\% noise and put them in a \code{DSD_Memory}. This object is used
...
produces a similar, almost regular pattern.
...
micro-clusters (e.g., assign the point to the closest center).
...
better value. We can compare purity here since the number 
...
Comparing algorithms on evolving stream is similarly easy in \pkg{stream}. 
...
First we create a stream which stores 5000 data points in memory.
...
cluster 5000 data points and evaluate using the corrected Rand index 
with a horizon of 250 points. This produces a list with $5000/250=20$ 
...
%animate_cluster(dsc, stream, n=5000, horizon=250, type="both", evaluationMeasure="crand", evaluationAssignmentMethod="model", evaluationType="macro", assign=T, ylim=c(0,1))
...
corrected Rand index values.
The matrix is visualized as a line plot and we add a boxplot comparing the 
distributions of the evaluation measure.
...
a study for clustering large data sets using an early version of 
...
and we have recorded some key statistics averaged over 10000 point intervals.
Figure~\ref{figure:intrusion}(a) reports clustering speed in 1000s of points
per second (on a laptop with a Intel Core i5-4300U at 1.9 GHz and 
8GB main memory running Ubuntu 14.10). 
Clustering 4 million data points took about 67 seconds.
Note that this measurement only includes the clustering and not the time it takes to access the data online.
The processing speed varies greatly between about 7000 and 357,000 points
per second with a median speed of 294,000 points per second.
The speed depends on the number of micro clusters which need 
to be maintained. 
Figure~\ref{figure:intrusion}(b) shows the number of micro-clusters
used by the algorithm. The maximal number of micro clusters is
12,310 which occurs in the first half of the data stream.
This number is directly related to the memory used by the algorithm. For the used 34 dimensional data set, each micro-cluster
occupies only 16 bytes (without attraction) 
or 560 bytes (with attraction)
which leads even with the overhead of the storage data structure 
to only a negligible memory consumption of a few MB.
...
Figure~\ref{figure:intrusion}(c) shows the number of different classes (normal and different types of intrusions) in 10000 point segments. 
...
%is implemented in \proglang{R} and thus slow (it took 37.5 minutes to cluster all the data), it still can be used to cluster
%large data sets in reasonable time. In order to improve run time, 
%we are working on reimplementing several algorithms in \proglang{C++}.
...
are produced at slower speeds. Another important application 
of \pkg{stream} 
is for processing 
data which does not fit in main memory point by point.
