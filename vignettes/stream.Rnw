...
which is not static, but the result of a 
...
for data stream algorithms, R currently does not have an infrastructure 
...
A DSD
can also be scaled by wrapping it in a stream class using:
\begin{itemize}
\item \code{DSD_ScaleStream}, a
DSD class that wraps a DSD and scales it using \code{scale} in \pkg{base}.
\end{itemize}
...
\begin{itemize}
\item
\code{DSC_Kmeans}, interface to R's $k$-means implementation or a a version of 
$k$-means where the data points 
