...
Very recently \pkg{RMOA}~\citep{stream:Wijffels:2014} was introduced. Th package interfaces data stream classification algorithms from the MOA framework (see existing tools discussed in Section~\ref{sec:background:moa}), however, instead of streaming data it focuses on static data sets that do not fit into main memory.
...
Section~\ref{sec:example} we provides a comprehensive example performing
an experimental comparison of several clustering algorithms.
...
make the introduce of the framework and its components
easier to follow.
...
We using the \proglang{S3}~class system~\citep{stream:Chambers:1992}
...
Details for developers interested in
can be found
in package's vignette and manual pages~\cite{stream:stream:2014}.
...
\item \code{DSD_ReadCSV} reads data in CSV format line by line 
from files or open connections and makes it available in a streaming fashion. 
...
in a blocking or non-blocking fashion.
...
\item {A data generating function} \code{get_points(x, n = 1, outofpoints = c("stop", "warn", "ignore") , ...)}. 
...
does not provide more points at this time). For \code{"warn"} and \code{"ignore"} all (possible zero) available points are returned. 
For clustering data,
...
classification (DSCClass) and frequent pattern mining (DSFPM).}
...
by the other tasks and we will discuss them in the context of clustering.
...
combine a micro and a macro-clustering algorithm.
...
\item
\code{update(dsc, dsd, n=1, verbose = FALSE, ...)} which accepts a DSC object and a DSD object. It takes $n$
...
where \code{n} data points are taken from \code{dsd} and assigned to a 
cluster in the clustering in \code{dsc} using \code{get_assignment()}. 
...
specified, all
available measures are calculated. We use only a small number of points 
since calculating some measures is expensive.
...
following examples that reclustering will improve the corrected Rand. 
...
represented by a value of \code{NA}. For silhouette calculation we 
replace the \code{NA}s with 0 to make the unassigned (noise) points its own 
cluster. Also, the silhouette is only calculated for a small number of points and not the whole stream.
...
\section{Sample application: Experimental comparison} \label{sec:example}
...
tNN (threshold nearest-neighbors).
...
Package \pkg{stream} is a data stream modeling framework in \proglang{R} that 
provides both
...
Furthermore, the presented infrastructure can be extended by adding new 
data sources and algorithms, or by defining a whole new data stream mining 
task. We have abstracted each 
...
classification (incorporating the algorithms interfaced by \pkg{RMOA}~\cite{stream:Wijffels:2014}) 
...
and were supported in part by the U.S. National
