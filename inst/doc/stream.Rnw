...
\begin{itemize}
\item \code{DSC_DBSCAN} implements 
    the weighted version of DBSCAN by \cite{Ester96adensity-based}
...
we have set the parameters such that
the number 
...
we evaluate and
cluster 5000 data points. For the evaluation we use the corrected Rand index 
